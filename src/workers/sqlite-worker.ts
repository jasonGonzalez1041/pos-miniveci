...
// ==================== SECURITY LAYER 1: BRANDED TYPES ====================

/**
 * Branded type for validated script paths to prevent arbitrary script loading
 * 
 * This type ensures compile-time safety by requiring explicit validation
 * before any script can be loaded via importScripts(). The brand prevents
 * accidental casting of arbitrary strings to validated paths.
 * 
 * @example
 * ```typescript
 * // ❌ This will not compile
 * const path: ValidatedScriptPath = "/malicious-script.js";
 * 
 * // ✅ This is the correct way
 * const path = "/sql-wasm.js";
 * if (isValidScriptPath(path)) {
 *   await secureLoadScript(path); // path is now ValidatedScriptPath
 * }
 * ```
 */
...
/**
 * Branded type for sanitized SQL operations to prevent SQL injection
 * 
 * This type ensures that all SQL queries have been through sanitization
 * before execution. The brand prevents passing raw user input directly
 * to database operations.
 * 
 * @example
 * ```typescript
 * // ❌ This will not compile
 * const query: SafeSQL = "DROP TABLE users; --";
 * 
 * // ✅ This is the correct way
 * const userInput = "SELECT * FROM products";
 * const safeQuery = sanitizeSQL(userInput); // Returns SafeSQL
 * db.exec(safeQuery);
 * ```
 */
...
/**
 * Template literal type for allowed SQL operations
 * Restricts SQL commands to safe, predefined patterns
 */
type AllowedSQLOperation = 
  | `SELECT ${string}`
  | `INSERT INTO ${string}`
  | `UPDATE ${string}`
  | `DELETE FROM ${string}`
...
const ALLOWED_SCRIPTS = {
  '/sql-wasm.js': {
    ...
    requiredGlobals: ['initSqlJs'] as const,
 ...
 */
function isValidScriptPath(path: string): path is ValidatedScriptPath {
 ...
 */
async function secureLoadScript(scriptPath: string): Promise<void> {
  // STEP 1: Static type validation
  if (!isValidScriptPath(scriptPath)) {
    throw new SecurityError(`Script path failed validation: ${scriptPath}`);
  }
  
  const validatedPath = scriptPath as ValidatedScriptPath;
  const scriptConfig = ALLOWED_SCRIPTS[validatedPath as keyof typeof ALLOWED_SCRIPTS];
  ...
  try {
    ...
    if (!response.ok) {
      throw new WorkerInitializationError(
        `Script fetch failed: ${response.status} ${response.statusText}`
      );
    }
    ...
    try {
      ...
      for (const globalName of scriptConfig.requiredGlobals) {
        if (typeof (self as any)[globalName] !== 'function') {
    ...
    throw new WorkerInitializationError(`Failed to load script: ${validatedPath}`, error as Error);
  }
}

// ==================== BUSINESS LOGIC WITH SECURITY INTEGRATION ====================

const log = (...args: unknown[]) => console.log('[SQLite Worker]', ...args);
const error = (...args: unknown[]) => console.error('[SQLite Worker]', ...args);

let db: any = null; // Will be typed properly after SQL.js loads
...
async function initializeSQLite(): Promise<void> {
  try {
    ...
    const initSqlJs = (self as any).initSqlJs as Function;
  ...
  try {
    ...
    switch (type) {
      case 'exec': {
        ...
        result = execResult.length > 0 ? execResult[0].values.map((row: unknown[]) => {
      ...
      case 'selectAll': {
        ...
        result = selectResult.length > 0 ? selectResult[0].values.map((row: unknown[]) => {
          const obj: Record<string, unknown> = {};
